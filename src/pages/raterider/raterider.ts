...
export class RateriderPage {
  ...
  submitRating() {
    if (!this.rateRequest.review || !this.rateRequest.review.length) {
      ...
      let subscription: Subscription = this.service.rateUser(window.localStorage.getItem(Constants.KEY_TOKEN), this.ap.provider.user_id, this.rateRequest).subscribe(res => {
        console.log(res);
        window.localStorage.setItem("rate" + this.ap.id, String(this.rateRequest.rating));
